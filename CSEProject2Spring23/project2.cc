...
void getUseless(){
    //declare two arrays storing generating symbols and reachable symbols
    bool generateArr[symbolSize];
    bool reachableArr[symbolSize];
    bool isChanged = true;

    //assign all terminal and hash to generate true
    ...
    for(auto &i : ruleList){
        if(!i.second.empty()){
            ...
            //empty body means hash which will be true
            gen = true;
        }
    ...
    //check if the start symbol is in ruleGen and ruleGen is not empty
    if(!ruleGen.empty() && ruleGen[0].first == ruleList[0].first){
        //assign start symbol to reachable
